...
If a citation entry carries `"code": "not_found"`, report that the query did
not resolve. This does not establish that the reference is absent from the
corpus: refine the author/year query or use a known work ID. Do not fabricate
a citation. If an entry carries
