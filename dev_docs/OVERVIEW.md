...
- **Docling path (the common case).** The pipeline sets `generate_picture_images=True` but leaves `images_scale` at its default of **`1.0`** (`pipeline/extract.py:63-70`), and docling renders picture crops at **`72 × images_scale` dpi** — i.e. **72 dpi** (verified in the project `corpus` env; the dpi base is `docling_core/types/doc/document.py`, `ImageRef.from_pil(…, dpi=round(72 * scale))`). Critically, this is a **fixed-scale rasterization, not preservation of the source's native embedded resolution** — a born-digital paper carrying a high-resolution embedded figure is still captured at 72 dpi. **This is the single biggest figure-quality lever and the most likely first optimization**: raising `images_scale` to 2–4× yields 144–288 dpi crops at a disk/CPU cost (docling's own picture classifier already renders at scale 2.0 internally, so 72 dpi for the *saved* crop is conservatively low).
