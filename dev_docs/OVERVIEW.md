...
- **Docling path (the common case).** The pipeline sets `generate_picture_images=True` and `images_scale` from the `figures.images_scale` config key (`pipeline/extract.py:63-78`), and docling renders picture crops at **`72 × images_scale` dpi** (the dpi base is `docling_core/types/doc/document.py`, `ImageRef.from_pil(…, dpi=round(72 * scale))`). The default is **`2.0` → 144 dpi** (#121); it was docling's effective `1.0` → **72 dpi** before v0.6, which looked grainy upscaled into print (#121). This is a **fixed-scale rasterization, not preservation of the source's native embedded resolution** — a born-digital paper carrying a high-resolution embedded figure is still captured at the configured scale. It remains **the biggest figure-quality lever**: raise `figures.images_scale` to 3–4 (216–288 dpi) for publication-bound corpuscles, at quadratic disk cost. The new default only affects future ingests; an existing bundle is lifted in place by `backfill_figure_dpi.py` (re-renders each figure from its stored bbox + `processed.pdf` at a target scale, no docling re-run).
