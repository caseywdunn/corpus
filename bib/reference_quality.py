...
def classify(conn, ref):
    ...
    panel_run = re.search(r"\bA\s*,\s*B\s*,\s*C\s*[,;:]", text)
    if panel_run and re.search(r"\b(?:views?|panels?)\b", text, re.IGNORECASE):
    ...
    return ("quarantined_fragment", reasons) if reasons else ("usable", [])
