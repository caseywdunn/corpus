...
counts, and latency without touching MCPServer's schema generation, and
...
def test_call_tool_is_instrumented():
    # The interceptor must be installed on the live MCPServer instance,
...
def test_dispatch_error_is_recorded_and_reraised():
    ...
    # ``context`` became a required positional on ToolManager.call_tool in
    # mcp 2.0 (#156); the real dispatch path builds it per request.
    ctx = Context(mcp_server=mcp, subscriptions=mcp._subscriptions)
    with pytest.raises(Exception):
        asyncio.run(mcp._tool_manager.call_tool(name, {}, ctx))
