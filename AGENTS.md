...
| `skills/` | General, **must stay group-agnostic** | Claude Code skills shipped as a plugin (`.claude-plugin/`). A skill may import public functions from `pipeline/`; the product never imports a skill (`tests/test_import_direction.py`). Collection-specific judgment belongs in the *library repo*, not in a skill here — the same rule that keeps the rest of this repo corpus-agnostic. The plugin must stay installable without the `corpus` package or an MCP server. Layout, frontmatter contract and checklist: CONTRIBUTING.md, "Adding a skill". |
